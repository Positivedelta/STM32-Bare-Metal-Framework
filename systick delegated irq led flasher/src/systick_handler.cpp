...
SysTick& SysTick::getInstance(const uint32_t timeBase, Listeners&& irqListeners, const uint8_t priority)
{
    static SysTick instance = SysTick(timeBase, std::move(irqListeners), priority);
    return instance;
}

SysTick::SysTick(const uint32_t timeBase, Listeners&& irqListeners, const uint8_t priority)
{
